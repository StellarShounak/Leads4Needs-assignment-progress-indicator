...
function App() {
  ...
  const [WrongAnsScore, setWrongAnsScore] = useState(0);
  ...
  function handleAnswer(isCorrect) {
    ...
    if (isCorrect) {
      ...
      setCorrectAnsScore(correctAnsScore+25)
         ...
         setWrongAnsScore(WrongAnsScore+25)
    }
  ...
  return (
    <>
        ...
        <div className="mb-5 d-flex flex-row justify-content-between w-full">
          ...
          <div className="w-100 mt-2">
                <ProgressBar variant="danger" now={WrongAnsScore} className="w-100"/>
          <div className="text-dark">{WrongAnsCount} / {questions.length}</div>
          </div>
        </div>
